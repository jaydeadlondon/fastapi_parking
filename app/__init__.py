...
def create_app():
    ...
    def exit_parking():
        ...
        entry = ClientParking.query.filter_by(client_id=client_id, parking_id=parking_id, time_out=None).first()
        if not entry:
        ...
        db.session.commit()
        return jsonify({"message": "Выезд разрешён", "time_out": entry.time_out.isoformat()})
